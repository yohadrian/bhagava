...
$(document).ready(function() {
	...
	if(!empty($_GET['id']))
		{?>
			...
			document.getElementById("sales_order_header_id").value="<?php echo $_GET['id'];?>";
			<?php
				$sqltab	="select * from sales_order_header where no_order='$_GET[id]'";
				$restab	= mysqli_query(mysqli_connect("localhost","root","","bhagavagallery"),$sqltab);
				$rowtab=mysqli_fetch_array($restab)
			?>
			document.getElementById("tglorder").value="<?php echo $rowtab['tgl_order'];?>";
	...
	$('#edit').click( function () {
			...
			$.ajax({		
                        ...
                        data: datatesnew[1],
						success: function () {
										window.location = "salesorder.php?id="+datatesnew[1]+"&sales_order_header_id="+datatesnew[0];
						}
                    });
		...
		<thead>
		 ...
		 <tr id="new_data_detail_insert">
			...
			<td>
				...
				<input type="text" class="form-control"  name="price[]" id="price[]" style="width:100%" readonly required value="0">
			</td>
			<td>
